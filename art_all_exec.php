	...
	if(isset($_POST['a']) && isset($_POST["phpu"])){
		...
		if($result->num_rows > 0){
			...
			while ($row = $result->fetch_assoc()){
				...
				$output .= "<a href='/articles/".$posttime."/".$written_by_original."' style='color: #000;'><div id='nev_rel_holder_ph_e' class='pcmacsm'>".$cover."<div id='new_inner_div_'><p style='margin-top: 10px; font-size: 14px;'><b>Author: </b>".$written_by."<br><b>Title: </b>".$title."</div></div></a>";
			}
			echo $output;
			exit();
		} else {
			// No results from search
			echo "<p style='text-align: center; margin: 0; padding: 10px; color: #999;'>Unfortunately, there are no results found</p>";
			exit();
		}
	}
?>