	...
	if(isset($_POST['g'])){
		...
		if($result->num_rows > 0){
			while ($row = $result->fetch_assoc()){
			    ...
			    $output .= "<a href='/group/".$gnameori."' style='color: #000;'><div id='nev_rel_holder_ph_e' class='pcmacsm'><div style='background-image: url(\"".$pcurl."\"); background-repeat: no-repeat; background-position: center; background-size: cover; width: 56px; height: 56px; float: right; margin-top: 8px; border-radius: 50%;'></div><div id='new_inner_div_'><p style='margin-top: 10px; font-size: 14px;'><b>Name: </b>".$gname."<br><b>Creator: </b>".$creator."<br><b>Established: </b>".$cd." (".$uds." ago)</p></div></div></a>";
			}
