<?php
  ...
  */
  ...
  list($paginationCtrls, $limit) = pagination($conn, $sql_s, 'si', $url_n, $u, $ar); 
	...
	$status_ui = '<textarea id="statustext_" onfocus="showBtnDiv()" placeholder="'.$txtMsg.'" class="user_status"></textarea>';
	...
	$status_ui .= '<div id="btns_SP" class="hiddenStuff" style="width: 90%;">';
		$status_ui .= '<span id="swithspan"><button id="statusBtn" class="btn_rply" onclick="postToStatus(\'status_post\',\'a\',\''.$u.'\',\'statustext_\')">Post</button></span>';
		$status_ui .= '<img src="/images/camera.png" id="triggerBtn_SP_" onclick="triggerUpload(event, \'fu_SP\')" width="22" height="22" title="Upload A Photo" class="triggerBtnreply" />';
		$status_ui .= '<img src="/images/emoji.png" width="22" class="triggerBtn" height="22" title="Send emoticons" id="emoji" onclick="openEmojiBox()">';
	...
	while ($row = $result->fetch_assoc()) {
		...
		$stmt->fetch();
		$stmt->close();
		$user_image = '<a href="/user/'.$author.'/"><div data-src=\''.$friend_pic.'\' style="background-repeat: no-repeat; '.$mgin.' background-size: cover; background-position: center; width: 50px; height: 50px; display: inline-block; margin-bottom: 5px;" class="tsrhov bbmob lazy-bg"></div><div class="infotsrdiv"><div data-src=\''.$friend_pic.'\' style="border-radius: 50%; background-repeat: no-repeat; background-size: cover; background-position: center; width: 60px; height: 60px; display: inline-block; float: left;" class="lazy-bg"></div><span style="float: left; margin-left: 2px;"><u>'.$funames.'</u>&nbsp;'.$isonimg.'<br><img src="/images/pcountry.png" width="12" height="12">&nbsp;'.$fuco.'<br><img src="/images/udist.png" width="12" height="12">&nbsp;Distance: '.$dist.' km<br><img src="/images/fus.png" width="12" height="12">&nbsp;Friends: '.$numoffs.'</span></div></a>';
		$data = $row["data"];
		$data_old = $row["data"];
	    ...
	    if($result2->num_rows > 0){
	        while ($row2 = $result2->fetch_assoc()) {
				...
				if($isLike_reply == true){
					...
					$likeButton_reply = '<a href="#" onclick="return false;" onmousedown="toggleLike_reply(\'like\',\''.$statusreplyid.'\',\'likeBtn_reply_'.$statusreplyid.'\')"><img src="/images/nf.png" width="18" height="18" class="like_unlike" title="Like"></a>';
	     	...
	     	// Count the replies
		    $b = "b";
		    $sql = "SELECT COUNT(id) FROM article_status WHERE type = ? AND account_name = ? AND osid = ? AND artid = ?";
		    $stmt = $conn->prepare($sql);
		...
		if($isFriend == true || $log_username == $u){
			...
			$statuslist .= '<div id="btns_SP_reply_'.$statusid.'" class="hiddenStuff rply_joiner">';
				...
				$statuslist .= '<img src="/images/camera.png" id="triggerBtn_SP_reply_" class="triggerBtnreply" onclick="triggerUpload_reply(event, \'fu_SP_reply\')" width="22" height="22" title="Upload A Photo" />';
			...
			$statuslist .= '<div id="standardUpload_reply" class="hiddenStuff">';
				...
				$statuslist .= '</form>';
			$statuslist .= '</div><div class="clear"></div>';
			}
		}
?>
