	...
	if(isset($_POST['u']) && isset($_POST["imp"])){
		...
		if($result->num_rows > 0){
			while ($row = $result->fetch_assoc()){
			    ...
			    $output .= "<a href='/user/".$unameori."/' style='color: #000;'><div id='nev_rel_holder_ph_e' class='pcmacsm'><div style='background-image: url(\"".$pcurl."\"); background-repeat: no-repeat; background-position: center; background-size: cover; width: 45px; height: 45px; float: right; margin-top: 6px; border-radius: 50%;'></div><div id='new_inner_div_'><p style='margin-top: 10px; font-size: 14px;'><b>Username: </b>".$uname."<br><b>Country: </b>".$country."<br></p></div></div></a>";
			}
			...
			echo "<p style='color: #999; text-align: center;'>Unfortunately, there are no results found ...</p>";
			echo $output;
			exit();
		}
	}
