		...
		if($_POST['type'] == "like"){
			...
			if($row_count1 > 0){
				...
				for($i = 0; $i < count($friends); $i++){
					...
					$note = $log_username.' liked a reply on: <br /><a href="/user/'.$accname.'#reply_'.$id.'/">Check it now</a>';
				...
				for($i = 0; $i < count($diffarr); $i++){
					...
					$stmt->close();
					$app = "Status Reply Like | your following, ".$log_username.", liked a reply on: <img src='/images/likeb.png' class='notfimg'>";
					$note = '<a href="/user/'.$accname.'#reply_'.$id.'/">'.$accname.'&#39;s profile</a>';
					// Insert into database
					$sql = "INSERT INTO notifications(username, initiator, app, note, date_time) VALUES(?,?,?,?,NOW())";
					$stmt = $conn->prepare($sql);
					$stmt->bind_param("ssss",$friend,$log_username,$app,$note);
