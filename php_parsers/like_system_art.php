...
<?php
	if(isset($_POST['type']) && isset($_POST['id'])){
		...
		$stmt->close();
		if($_POST['type'] == "like"){
			$sql = "SELECT COUNT(id) FROM art_stat_likes WHERE username=? AND status=? AND artid=? LIMIT 1";
			$stmt = $conn->prepare($sql);
			...
			if($row_count1 > 0){
				...
				$stmt->close();
				$sql = "INSERT INTO art_stat_likes(username, status, artid, like_time)
						VALUES (?,?,?,NOW())";
				$stmt = $conn->prepare($sql);
				$stmt->bind_param("sii",$log_username,$id,$parid);
				$stmt->execute();
				...
				for($i = 0; $i < count($friends); $i++){
					...
					$stmt->close();
					$app = "Article Status Post Like <img src='/images/reply.png' class='notfimg'>";
					$note = $log_username.' liked a post on an article: <br /><a href="/articles/'.$ptime.'/'.$log_username.'#status_'.$id.'">Check it now</a>';
				...
				for($i = 0; $i < count($diffarr); $i++){
					...
					$ptime = base64url_encode($ptime,$hshkey);
					$stmt->close();
					$app = "Article Status Post Like | your following, ".$log_username.", liked an article post";
			...
			if($row_count1 > 0){
				$stmt->close();
		        $sql = "DELETE FROM art_stat_likes WHERE username=? AND status=? AND artid=? LIMIT 1";
				$stmt = $conn->prepare($sql);
				$stmt->bind_param("sii",$log_username,$id,$parid);
				$stmt->execute();
				$stmt->close();
