<?php
  ...
  if(isset($_POST['type']) && isset($_POST['id'])){
    ...
    if($_POST['type'] == "like"){
      $sql = "SELECT COUNT(id) FROM photo_reply_likes WHERE username=? AND reply=? AND photo=?
        LIMIT 1";
      ...
      if($row_count1){
        ...
        $note = $log_username.' liked a reply below a photo: <br />
          <a href="/photo_zoom/'.$phh.'/'.$log_username.'#reply_'.$photLike->p1.'">
            Check it now</a>';
