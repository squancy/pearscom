...
<?php
	if(isset($_POST['type']) && isset($_POST['id']) && isset($_POST['group'])){
		...
		$stmt->close();
		if($_POST['type'] == "like"){
			$sql = "SELECT COUNT(id) FROM group_reply_likes WHERE username=? AND gpost=? AND gname = ? LIMIT 1";
			$stmt = $conn->prepare($sql);
			...
			if($row_count1 > 0){
				...
				for($i = 0; $i < count($friends); $i++){
					...
					$note = $log_username.' liked a comment on '.$gname.' group: <br /><a href="/group/'.$gname.'#status_'.$id.'">Check it now</a>';
				...
				for($i = 0; $i < count($diffarr); $i++){
					$friend = $diffarr[$i];
					$app = "Group Reply Like | your following, ".$log_username.", liked a group post on ".$gname." group <img src='/images/likeb.png' class='notfimg'>";
			...
			if($row_count1 > 0){
				$stmt->close();
		        $sql = "DELETE FROM group_reply_likes WHERE username=? AND gpost=? AND gname = ? LIMIT 1";
				$stmt = $conn->prepare($sql);
				$stmt->bind_param("sis",$log_username,$id,$gname);
				$stmt->execute();
				$stmt->close();
