<?php
	...
	if($last != 1){
		...
		if ($pagenum > 1) {
	        $previous = $pagenum - 1;
			$paginationCtrls .= '<a href="/user/'.$u.'&pn='.$previous.'#posts">Previous</a> &nbsp; &nbsp; ';
			// Render clickable number links that should appear on the left of the target page number
			for($i = $pagenum-4; $i < $pagenum; $i++){
				if($i > 0){
			        $paginationCtrls .= '<a href="/user/'.$u.'&pn='.$i.'#posts">'.$i.'</a> &nbsp; ';
				}
		    }
	    }
	    ...
	    if ($pagenum != $last) {
	        $next = $pagenum + 1;
	        $paginationCtrls .= ' &nbsp; &nbsp; <a href="/user/'.$u.'&pn='.$next.'#posts">Next</a> ';
	    }
	}
	...
	$status_ui = ''.$toDis.'
			<textarea id="statustext" onfocus="showBtnDiv()" placeholder="'.$wmes.'" class="user_status"></textarea>';
        ...
        $status_ui = "<p style='color: #999;' class='txtc'>To leave a comment please <a href='/login'>log in</a></p>";
    }
	?>
	<?php
...
?>
	...
	function deleteStatus(id, status) {
	  ...
	  var xhr = ajaxObj("POST", "/php_parsers/status_system.php");
	...
	function deleteReply(result, data) {
	  ...
	  var res = ajaxObj("POST", "/php_parsers/status_system.php");
	...
	function shareStatus(type) {
	  var request = ajaxObj("POST", "/php_parsers/status_system.php");
	...
	function toggleLike(e, o, t) {
	  var result = ajaxObj("POST", "/php_parsers/like_system.php");
	...
	function toggleLike_reply(e, o, t) {
	  var result = ajaxObj("POST", "/php_parsers/like_reply_system.php");
	  result.onreadystatechange = function() {
	    if (1 == ajaxReturn(result)) {
