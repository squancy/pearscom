<?php
	...
	if(isset($_POST['a'])){
		...
		$sql = "SELECT a.*, u.*
		        ...
		        WHERE a.written_by LIKE ? OR a.title LIKE ? OR a.category LIKE ? OR a.tags LIKE ?
				ORDER BY a.written_by ASC LIMIT 100";
		$stmt = $conn->prepare($sql);
		$stmt->bind_param("ssss",$a_search,$a_search,$a_search,$a_search);
		$stmt->execute();
		...
		if($result->num_rows > 0){
			...
			while ($row = $result->fetch_assoc()){
				...
				$output .= "<a href='/articles/".$posttime."/".$written_by_original."' style='color: #000;'><div id='nev_rel_holder_ph_e' class='pcmacsm'>".$cover."<div id='new_inner_div_'><p style='margin-top: 10px; font-size: 14px;'><b>Author: </b>".$written_by."<br><b>Title: </b>".$title."</p></div></div></a>";
			}
			echo $output;
			exit();
		} else {
