...
if (active === false) {
  ...
  setTimeout(function() {
    lazyImages.forEach(function(lazyImage) {
      if (((lazyImage.getBoundingClientRect().top <= window.innerHeight && lazyImage.getBoundingClientRect().bottom >= 0) && getComputedStyle(lazyImage).display !== "none") || isFrom) {
          if((!lazyImage.classList.contains('noRound') && !lazyImage.classList.contains('recbgs')) || isFrom){
...
if(document.getElementsByClassName('innerView')[0] != null){
    document.getElementsByClassName('innerView')[0].addEventListener('scroll', lazyLoad);
}

if(document.getElementById('user_template_img') != null){
    document.getElementById('user_template_img').addEventListener('click', function(e){
        lazyLoad(true);
    });
}
